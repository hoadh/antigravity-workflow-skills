...
# Installs all dependencies for Antigravity agent skills
#
...
install_node_deps() {
    ...
    # payment-integration
    if [ -d "$SCRIPT_DIR/payment-integration/scripts" ] && [ -f "$SCRIPT_DIR/payment-integration/scripts/package.json" ]; then
        print_info "Installing payment-integration dependencies..."
        (cd "$SCRIPT_DIR/payment-integration/scripts" && npm install --quiet)
        print_success "payment-integration dependencies installed"
    fi

    # Optional: Shopify CLI
    if [ -d "$SCRIPT_DIR/shopify" ]; then
        if [[ "$SKIP_CONFIRM" == "true" ]]; then
            print_info "Skipping Shopify CLI installation (optional)"
        else
...
main() {
    echo ""
    print_header "Antigravity Agent Skills Installation"
    print_info "OS: $OS"
