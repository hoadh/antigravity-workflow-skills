...
def find_api_key() -> Optional[str]:
    ...
    Uses ~/.agent/scripts/resolve_env.py for consistent resolution across all skills.
                ...
                for i, part in enumerate(response.candidates[0].content.parts):
                    if part.inline_data:
                        ...
                        if file_path:
                            ...
                            # Find project root (look for .git or .agent directory)
                            script_dir = Path(__file__).parent
                            project_root = script_dir
                            for parent in [script_dir] + list(script_dir.parents):
                                if (parent / '.git').exists() or (parent / '.agent').exists():
...
def batch_process(
    ...
    if not api_key:
        ...
        print("2. Show hierarchy: python ~/.agent/scripts/resolve_env.py --show-hierarchy --skill ai-multimodal")
