...
.env loading order: process.env > skill/.env > skills/.env > .agent/.env
"""
...
class BetterAuthInit:
    ...
    def _load_env_files(self) -> Dict[str, str]:
        ...
        Loading order: process.env > skill/.env > skills/.env > .agent/.env
        ...
        env_paths = [
            self.project_root / '.agent' / ".env",
            self.project_root / '.agent' / "skills" / ".env",
            skill_dir / ".env",
        ]
