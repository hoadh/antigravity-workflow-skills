...
1. **Search Strategy**:
   - **Gemini Toggle**: Check `.agents/.agent.json` (or `~/.gemini/antigravity/.agent.json`) for `skills.research.useGemini` (default: `true`). If `false`, skip Gemini and use WebSearch.
   - **Gemini Model**: Read from `.agents/.agent.json` (or `~/.gemini/antigravity/.agent.json`): `gemini.model` (default: `gemini-3-flash-preview`)
