...
Read model from `.agent/.ck.json`: `gemini.model` (default: `gemini-3-flash-preview`)
...
Gemini CLI reads MCP servers from `.gemini/settings.json`. Create a symlink to `.agent/.mcp.json`:
...
ln -sf .agent/.mcp.json .gemini/settings.json

# Create symlink (Windows - requires admin or developer mode)
mklink .gemini\settings.json .agent\.mcp.json
```
...
1. **Configuration Loading**: Reads `.gemini/settings.json` (symlinked to `.agent/.mcp.json`)
...
Should show symlink pointing to `.agent/.mcp.json`.
